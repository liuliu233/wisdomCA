...
layui.define(['jquery', 'miniMenu', 'element', 'miniTab', 'miniTheme'], function(exports) {
  ...
  if (!/http(s*):\/\//.test(location.href)) {
    const tips = '请先将项目部署至web容器（Apache/Tomcat/Nginx/IIS/等），否则部分数据将无法显示';
  ...
  var miniAdmin = {
    ...
    renderLogo: function(data) {
      const html = '<a href="' + data.href + '"><img src="' + data.image + '" alt="logo"><h1>' + data.title + '</h1></a>';
    ...
    renderHome: function(data) {
      ...
      $('#layuiminiHomeTabIframe').html('<iframe width="100%" height="100%" scrolling="no" frameborder="no" border="0" marginwidth="0" marginheight="0"  src="' + data.href + '"></iframe>');
    },
    ...
    exitFullScreen: function() {
      const el = document;
      const cfs = el.cancelFullScreen || el.webkitCancelFullScreen || el.exitFullScreen;
    ...
    listen: function() {
      ...
      $('body').on('click', '[data-clear]', function() {
        ...
        // 判断是否清理服务端
        const clearUrl = $(this).attr('data-href');
      ...
      $('body').on('mouseenter', '.layui-nav-tree .menu-li', function() {
        ...
        if (isShow == 0 && tips) {
          ...
          window.openTips = layer.tips(tips, $(this), {
            ...
            skin: 'popup-tips',
            success: function(el) {
              const left = $(el).position().left - 10;
              $(el).css({left: left});
              element.render();
            }
          });
      ...
      $('body').on('click', '[data-check-screen]', function() {
        const check = $(this).attr('data-check-screen');
        if (check == 'full') {
